...
export default function AccidentReportForm({ onSubmitted, editReport, onEditCancel }: AccidentReportFormProps) {
  ...
  useEffect(() => {
    if (editReport) {
      ...
      // Set radio states based on loaded data
      ...
      setInsuranceType(insDate === 'Not Produced' ? 'Not Produced' : insDate === 'Not Applicable' ? 'Not Applicable' : insDate ? 'Valid' : 'Valid');
    } else {
      setEditingId(null);
    }
  }, [editReport]);

  // Auto-update parking brake when vehicle class changes
  useEffect(() => {
    const isMotorcycle = formData.vehicleClass?.toLowerCase().includes('motorcycle') || formData.vehicleClass?.toLowerCase().includes('motor cycle') || formData.vehicleClass?.toLowerCase().includes('two wheeler');
    if (isMotorcycle) {
      updateField('parkingBrakeLackLubrication', 'Not Applicable');
    } else if (formData.parkingBrakeLackLubrication === 'Not Applicable') {
  ...
  const updateField = (field: keyof AccidentReportFormData, value: string) => {
    setFormData((prev) => {
      ...
      // Auto-sync inspectionPlace from roadDescription
  ...
  const toggleCheckbox = (field: keyof AccidentReportFormData) => {
    setFormData((prev) => ({ ...prev, [field]: prev[field] === 'Yes' ? '' : 'Yes' }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || (editingId ? 'Failed to update report' : 'Failed to create report'));
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <CardContent className="pt-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label htmlFor="crimeNo" className="text-slate-700 font-medium">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="U/S 106(BNS)">U/S 106(BNS)</SelectItem>
                  <SelectItem value="U/S 30(A)">U/S 30(A)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              <Label htmlFor="policeStation" className="text-slate-700 font-medium">
      ...
      <Accordion type="multiple" defaultValue={['section-1', 'section-2', 'section-3']} className="space-y-2">
        ...
        <AccordionItem value="section-1" className="border border-slate-300 rounded-lg overflow-hidden">
          <AccordionTrigger className="px-4 py-3 bg-slate-50 hover:bg-slate-100 text-slate-800 font-medium hover:no-underline">
        ...
        <AccordionItem value="section-3" className="border border-slate-300 rounded-lg overflow-hidden">
          ...
          <AccordionContent className="px-4 pb-4 pt-2 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <RadioGroup
                value={formData.fitnessCertExpiryDate === 'FC Not Produced' ? 'Not Produced' : formData.fitnessCertExpiryDate === 'Not Applicable' ? 'Not Applicable' : 'Select Date'}
                onValueChange={(v) => {
                  if (v === 'Select Date') {
              ...
              >
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="Select Date" id="fc-select-date" />
                  <Label htmlFor="fc-select-date" className="text-sm text-slate-600 cursor-pointer">Select Date</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="Not Produced" id="fc-not-produced" />
                  <Label htmlFor="fc-not-produced" className="text-sm text-slate-600 cursor-pointer">FC Not Produced</Label>
                </div>
                <div className="flex items-center space-x-2">
              ...
              {(formData.fitnessCertExpiryDate !== 'FC Not Produced' && formData.fitnessCertExpiryDate !== 'Not Applicable') && (
                <Input
          ...
          <AccordionContent className="px-4 pb-4 pt-2 space-y-4">
            ...
            <div>
              ...
              <div className="border border-slate-200 rounded-lg p-4 mb-4">
                <h4 className="font-medium text-slate-700 mb-3">If Hydraulic:</h4>
              ...
              <div className="border border-slate-200 rounded-lg p-4">
                <h4 className="font-medium text-slate-700 mb-3">If Mechanical:</h4>
                <div className="flex items-center gap-3">
                  <Checkbox
                    id="mech-no-failure"
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-center gap-3">
                      <Checkbox
                        id="pb-status"
                        checked={isMotorcycle ? formData.parkingBrakeLackLubrication === 'Not Applicable' : formData.parkingBrakeLackLubrication === 'No Brake Failure'}
                        onCheckedChange={() => {
                          if (isMotorcycle) {
                            updateField('parkingBrakeLackLubrication', 'Not Applicable');
                          } else {
                      ...
                      <Label htmlFor="pb-status" className="text-sm text-slate-600 font-medium">
                        {isMotorcycle ? 'Not Applicable' : 'NO Brake Failure'}
                      </Label>
                    </div>
                  );
                })()}
        ...
        {/* Section 5 - Steering, Tyres & Documents */}
        <AccordionItem value="section-5" className="border border-slate-300 rounded-lg overflow-hidden">
          ...
          <AccordionContent className="px-4 pb-4 pt-2 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <RadioGroup
                value={formData.permitValidity === 'Permit Not Produced' || formData.permitValidity === 'Not Produced' ? 'Not Produced' : formData.permitValidity === 'Not Applicable' ? 'Not Applicable' : 'Select Date'}
                onValueChange={(v) => {
                  if (v === 'Select Date') {
              ...
              >
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="Select Date" id="pv-select-date" />
                  <Label htmlFor="pv-select-date" className="text-sm text-slate-600 cursor-pointer">Select Date</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="Not Produced" id="pv-not-produced" />
                  <Label htmlFor="pv-not-produced" className="text-sm text-slate-600 cursor-pointer">Permit Not Produced</Label>
                </div>
                <div className="flex items-center space-x-2">
              ...
              {(formData.permitValidity !== 'Permit Not Produced' && formData.permitValidity !== 'Not Produced' && formData.permitValidity !== 'Not Applicable') && (
                <Input
        ...
        <AccordionItem value="section-8" className="border border-slate-300 rounded-lg overflow-hidden">
          ...
          <AccordionContent className="px-4 pb-4 pt-2 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label className="text-slate-700 font-medium">VCR No</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="text-slate-700 font-medium">VCR Date</Label>
                <Input
            ...
            <div className="space-y-2">
              <Label className="text-slate-700 font-medium">Copy to</Label>
              {(() => {
                ...
                return (
                  <>
                    <RadioGroup
                      ...
                      onValueChange={(v) => {
                        if (v === 'auto') {
                          updateField('copyTo', distName ? `DTO office, ${distName}` : '');
                        } else {
                          updateField('copyTo', '');
                        }
                      }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-2">
                <Label className="text-slate-700 font-medium">Inspector Name</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="text-slate-700 font-medium">Inspector Title</Label>
                <Input
                  value={formData.inspectorTitle || ''}
                  onChange={(e) => updateField('inspectorTitle', e.target.value)}
                  placeholder='e.g. "MOTOR VEHICLES INSPECTOR"'
                  className="border-slate-300"
                />
              </div>
              <div className="space-y-2">
                <Label className="text-slate-700 font-medium">Inspector Location</Label>
                <Input
                  value={formData.inspectorLocation || ''}
                  onChange={(e) => updateField('inspectorLocation', e.target.value)}
